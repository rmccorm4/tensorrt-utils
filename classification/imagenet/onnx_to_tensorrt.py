...
def main():
    ...
    parser.add_argument("-b", "--max_batch-size", type=int, default=32,
    ...
    parser.add_argument("--calibration-cache", default="calibration.cache",
    ...
    parser.add_argument("--max-calibration-size", type=int, default=512,
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(network_flags) as network, trt.OnnxParser(network, TRT_LOGGER) as parser:
        ...
        if args.int8:
            ...
            # Use calibration files from validation dataset if no cache exists
                ...
                if not args.calibration_data:
                    raise ValueError("ERROR: Int8 mode requested, but no calibration data provided. "
                                     "Please provide --calibration-data /path/to/calibration/files")
            ...
            builder.int8_calibrator = ImagenetCalibrator(calibration_files=calibration_files,
